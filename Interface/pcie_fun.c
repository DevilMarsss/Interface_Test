...
void c2h_transfer(unsigned int address, unsigned int size, unsigned char *buffer)
{
    ...
    //FlushFileBuffers(h_c2h0);//可能能刷新缓存数据到设备
    //UsSleep(1);
}
...
void user_write(unsigned int address, unsigned int size, unsigned char *buffer)
{
    ...
    //UsSleep(1);
    FlushFileBuffers(h_user);//可能能刷新缓存数据到设备
}
...
int read_fifo(unsigned int addr)
{
    ...
    //2021.8.21调试前
    //判断当前fifo中是否有数据帧，即若返回值为0则无数据，否则有数据， xdma_rw.exe user read 0x60000 -l 4
    ...
    user_write(0x60014, 4, (unsigned char *)&ack);//和逻辑握手，告知逻辑，软件已经判断到fifo中有数据，
    g_cnt14++;
...
//    user_write(0x50030, 4, (unsigned char *)&start);
    ...
    user_write(0x60014, 4, (unsigned char *)&clear);//握手结束
